...
{	
	...
	rndVal = (int64_t)( 1llu << (DEF_NFRACBITS - 1 + (32 - CSHIFT)) );
	...
	*(pcm + 0) = ClipToShort((int32_t)SAR64(sum1L, (32-CSHIFT)), DEF_NFRACBITS);
	...
	*(pcm + 16) = ClipToShort((int32_t)SAR64(sum1L, (32-CSHIFT)), DEF_NFRACBITS);
	...
	for (i = 15; i > 0; i--) {
		...
		vb1 += 64;
		*(pcm)       = ClipToShort((int32_t)SAR64(sum1L, (32-CSHIFT)), DEF_NFRACBITS);
		*(pcm + 2*i) = ClipToShort((int32_t)SAR64(sum2L, (32-CSHIFT)), DEF_NFRACBITS);
		pcm++;
	}
}
...
void PolyphaseStereo(short *pcm, int32_t *vbuf, const int32_t *coefBase)
{
	...
	*(pcm + 0) = ClipToShort((int32_t)SAR64(sum1L, (32-CSHIFT)), DEF_NFRACBITS);
	*(pcm + 1) = ClipToShort((int32_t)SAR64(sum1R, (32-CSHIFT)), DEF_NFRACBITS);
	...
	*(pcm + 2*16 + 0) = ClipToShort((int32_t)SAR64(sum1L, (32-CSHIFT)), DEF_NFRACBITS);
	*(pcm + 2*16 + 1) = ClipToShort((int32_t)SAR64(sum1R, (32-CSHIFT)), DEF_NFRACBITS);
	...
	for (i = 15; i > 0; i--) {
		...
		*(pcm + 0)         = ClipToShort((int32_t)SAR64(sum1L, (32-CSHIFT)), DEF_NFRACBITS);
		*(pcm + 1)         = ClipToShort((int32_t)SAR64(sum1R, (32-CSHIFT)), DEF_NFRACBITS);
		*(pcm + 2*2*i + 0) = ClipToShort((int32_t)SAR64(sum2L, (32-CSHIFT)), DEF_NFRACBITS);
		*(pcm + 2*2*i + 1) = ClipToShort((int32_t)SAR64(sum2R, (32-CSHIFT)), DEF_NFRACBITS);
		pcm += 2;
	}
}

 ...
 * Return:      none
 **************************************************************************************/
void SetBitstreamPointer(BitStreamInfo *bsi, int32_t nBytes, unsigned char *buf)
{
	/* init bitstream */
	bsi->bytePtr = buf;
...
static __inline void RefillBitstreamCache(BitStreamInfo *bsi)
{
	...
	if (nBytes >= 4) {
		bsi->iCache  = (uint32_t)(*bsi->bytePtr++) << 24; //fix tronix
		bsi->iCache |= (uint32_t)(*bsi->bytePtr++) << 16; //fix tronix
 ...
 **************************************************************************************/
int32_t CalcBitsUsed(BitStreamInfo *bsi, unsigned char *startBuf, int32_t startOffset)
{
	int32_t bitsUsed;
 ...
 **************************************************************************************/
int32_t CheckPadBit(MP3DecInfo *mp3DecInfo)
{
	FrameHeader *fh;
 ...
 **************************************************************************************/
int32_t UnpackFrameHeader(MP3DecInfo *mp3DecInfo, unsigned char *buf)
{
	...
	mp3DecInfo->nGranSamps = ((int32_t)samplesPerFrameTab[fh->ver][fh->layer - 1]) / mp3DecInfo->nGrans;
	 ...
	 */
	if (fh->brIdx) {
		mp3DecInfo->bitrate = ((int32_t)bitrateTab[fh->ver][fh->layer - 1][fh->brIdx]) * 1000;
	
		/* nSlots = total frame bytes (from table) - sideInfo bytes - header - CRC (if present) + pad (if present) */
		mp3DecInfo->nSlots = (int32_t)slotTab[fh->ver][fh->srIdx][fh->brIdx] - 
			(int32_t)sideBytesTab[fh->ver][(fh->sMode == Mono ? 0 : 1)] - 
			4 - (fh->crc ? 2 : 0) + (fh->paddingBit ? 1 : 0);
	}

	/* load crc word, if enabled, and return length of frame header (in bytes) */
	if (fh->crc) {
		fh->CRCWord = ((int32_t)buf[4] << 8 | (int32_t)buf[5] << 0);
		return 6;
	} else {
		fh->CRCWord = 0;
		return 4;
 ...
 **************************************************************************************/
int32_t UnpackSideInfo(MP3DecInfo *mp3DecInfo, unsigned char *buf)
{
